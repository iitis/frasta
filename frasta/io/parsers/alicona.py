...
def _decode_tag_bytes(raw: bytes) -> str:
    """Decode one Alicona tag field using ASCII-compatible semantics."""

    return raw.split(b"\x00", 1)[0].decode("latin-1").strip()


def _read_tag(handle) -> tuple[str, str]:
    """Read one fixed-size header tag from an AL3D stream."""
...
def _read_required_int(tags: dict[str, str], key: str) -> int:
    """Read one required integer-valued tag."""
...
def _read_required_float(tags: dict[str, str], key: str) -> float:
    """Read one required float-valued tag."""
...
def load_alicona_al3d(fname: str, progress_callback=None) -> Surface:
    """Load one Alicona Imaging ``.al3d`` file into a ``Surface``.

    Args:
        fname: Path to the input ``.al3d`` file.
        progress_callback: Optional callback receiving coarse progress values
            in the ``0-100`` range.

    Returns:
        Surface: Parsed surface with spacing converted to micrometers.

    Raises:
        FileNotFoundError: If the file does not exist.
        ValueError: If the file is not a valid AL3D file or the payload is
            malformed.
    """
    ...
    try:
        with open(fname, "rb") as handle:
            ...
            # AL3D rows are padded to 8-byte boundaries, so each row is read separately.
